...
def export_results_workbook(
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        t1_sup = _read_csv(tab_dir / "table1_supplement_diversity.csv")
        _write_df_sheet(writer, "04b_α多样性补充(Table1-补充)", t1_sup if t1_sup is not None else pd.DataFrame())
        _write_df_sheet(writer, SHEET_TABLE2, table2_fmt)
        factorial = _read_csv(tab_dir / "table2_factorial_delta_auc.csv")
        _write_df_sheet(writer, "05b_析因ΔAUC(Table2-附)", factorial if factorial is not None else pd.DataFrame())
        cont = _read_csv(tab_dir / "table2_continuous_extubation.csv")
        _write_df_sheet(writer, "05c_连续拔管时间(Table2敏感性)", cont if cont is not None else pd.DataFrame())
        supp_t2 = _read_csv(tab_dir / "table2_model_performance_supplementary.csv")
        _write_df_sheet(writer, "05d_补充模型(AE+MLP)", supp_t2 if supp_t2 is not None else pd.DataFrame())
        _write_df_sheet(writer, SHEET_TABLE3, table3_fmt)
        genus_w = _read_csv(tab_dir / "table_genus_wilcoxon.csv")
